    ...
    <div class="row-bootstrap m-0">
      ...
      <div class="col-12 col-md-8 half">
        <div v-if="getOptionLivraison.mode == 'retrait'">
          ...
          <p style="color: var(--color3); font-style: italic; margin-bottom: 20px">
            Vous avez choisi le retrait dans un de nos dépots, merci de préciser un
            dépot.
          </p>
          <ul class="depots-items">
            <li v-for="(item, index) in depots" 
              :key="index" 
              :class="selectedDeliveryAddress==item ? 'active' : ''"
              >
              <label class="flex">
                <div class="radio">
                  <input :id="item" :value="item" :name="item.label" v-model="selectedDeliveryAddress" type="radio"/>
                </div>
                <div>
                  <p class="name"><span>{{item.label}}</span></p>
                  <div class="flex">
        ...
        <div v-if="!sameAddress || getOptionLivraison.mode == 'retrait'">
          <form v-if="!sameAddress || getOptionLivraison.mode == 'retrait'" data-vv-scope="billingAddressForm" class="formInvoice">
            ...
            <div class="row-bootstrap">
              ...
              <div v-if="user && user.type == 'guest' && getOptionLivraison.mode == 'retrait'" class="col-12 col-md-6">
...
export default {
  ...
  watch: {
    selectedDeliveryAddress(newVal) {
      this.setOptionLivraison({...this.getOptionLivraison,depot: newVal ,mode: 'retrait'})
    },
    user() {
      this.preCompleteForm()
    }
